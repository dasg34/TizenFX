...
/// <param name="style">The widget style to use. See <see cref="CoreUI.UI.Widget.SetStyle" /></param>
    ...
    /// 
    /// Each radio button in a group must have a unique value. The selected button in a group can then be set or retrieved through the <see cref="CoreUI.UI.IRadioGroup.SelectedValue"/> property. This value is also informed through the <see cref="CoreUI.UI.IRadioGroup.ValueChangedEvent"/> event.
    /// 
...
#pragma warning disable CS1591
/// <since_tizen> 6 </since_tizen>
public static class RadioExtensionMethods {
    /// <since_tizen> 6 </since_tizen>
    public static CoreUI.BindableProperty<int> StateValue<T>(this CoreUI.UI.ItemFactory<T> fac, CoreUI.Csharp.ExtensionTag<CoreUI.UI.Radio, T>magic = null) where T : CoreUI.UI.Radio {
        return new CoreUI.BindableProperty<int>("state_value", fac);
    }
